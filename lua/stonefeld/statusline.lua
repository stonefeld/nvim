...
local function get_filepath()
	...
	return string.format(
		" %%#StlPurple#%s%s%%#StlBlue#%%t  %%#StlWhite#%s",
		filep,
		fn.has("win32") == 1 and "\\" or "/",
		icon
	)
end
...
function GetGitBranch()
	...
	if is_git_dir == "true" then
		GitBranch = string.format(
			" %%#StlGreen# %s  ",
			fn.trim(fn.system("git -C " .. fn.getcwd() .. " branch --show-current"))
		)
	else
		GitBranch = ""
	end
...
au("FileType", {
	pattern = { "qf", "netrw", "TelescopePrompt", "packer", "lspinfo", "mason" },
	callback = function()
